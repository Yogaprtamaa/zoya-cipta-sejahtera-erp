...
/**
 * POST /api/order/[id]/approve — { action: "approve"|"reject"|"ship", by?: "admin" }
 * Enforces approval threshold and moves stock to agent (consigned) on ship.
 */
export async function POST(req: Request, { params }: { params: Promise<{ id: string }> }) {
  const { id } = await params;
  const { action, by = "admin" } = await body<{ action: "approve" | "reject" | "ship"; by?: string }>(req);
  ...
  if (action === "reject") {
    ...
    const large = order.totalValue >= db.settings.approval_threshold;
    if (order.status === "admin_review") {
      order.status = large ? "pending_approval" : "approved";
    } else if (order.status === "pending_approval") {
      order.status = "approved";
    } else {
      return fail("Transisi approval tidak valid", "INVALID_TRANSITION");
    }
  } else if (action === "ship") {
